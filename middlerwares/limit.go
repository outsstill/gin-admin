...
func LimitIP(app *core.App, limit string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		// 针对 IP 限流
		key := limiterObj.GetKeyIP(c)
		if ok := limitHandler(c, app, key, limit); !ok {
			return
		}
		c.Next()
	}
...
func LimitPerRoute(app *core.App, limit string) gin.HandlerFunc {
	...
	return func(c *gin.Context) {
		...
		// 针对 IP + 路由进行限流
		key := limiterObj.GetKeyRouteWithIP(c)
		if ok := limitHandler(c, app, key, limit); !ok {
			return
		}
		c.Next()
...
func limitHandler(c *gin.Context, app *core.App, key string, limit string) bool {
